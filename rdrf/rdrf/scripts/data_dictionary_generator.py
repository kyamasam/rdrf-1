...
def decode(letters):
    return map(lambda s: s.decode("utf-8"), letters)
    ...
    def validation(self):
        ...
        if self.datatype == "STRING":
            ...
            if self.cde_dict["pattern"]:
                vals.append(
                    "Must conform to regular expression %s" % self.cde_dict["pattern"]
                )
...
class DataDefinitionReport(object):
    ...
    def _get_demographic_fields(self):
        fields = []
        fields.append(
            DemographicField(DemographicForm.SECTION_REGISTRY, "Centre", required=True)
        )
        ...
        fields.append(
            DemographicField(DemographicForm.SECTION_PATIENT_DETAILS, "Maiden name")
        )
        fields.append(
            DemographicField(
        ...
        fields.append(
            DemographicField(DemographicForm.SECTION_PATIENT_DETAILS, "Home Phone")
        )
        fields.append(
            DemographicField(DemographicForm.SECTION_PATIENT_DETAILS, "Mobile Phone")
        )
        fields.append(
            DemographicField(DemographicForm.SECTION_PATIENT_DETAILS, "Work Phone")
        )
        fields.append(
        ...
        fields.append(DemographicField(DemographicForm.HOME_ADDRESS, "Suburb/Town"))
        fields.append(
    ...
    def _get_consent_fields(self):
        ...
        def mk_consent(sec, field, required=False):
            fields.append(
                DemographicField(sec, field, "RANGE", BOOLEAN, required=required)
            )
    ...
    def __iter__(self):
        ...
        for demographic_field in self._get_demographic_fields():
            yield str(
                col
            ), "DEMOGRAPHICS", demographic_field.section, demographic_field.name, demographic_field.datatype, demographic_field.required, demographic_field.members, demographic_field.validation
            col += 1

        for field in self._get_consent_fields():
            yield str(
                col
            ), "CONSENTS", field.section, field.name, field.datatype, field.required, field.members, field.validation
            col += 1
        ...
        for form_dict in self.data["forms"]:
            ...
            for section_dict in form_dict["sections"]:
                if not section_dict["allow_multiple"]:
                    ...
                    for cde in cdes:
                        yield str(col), form_dict["name"], section_dict[
                            "display_name"
                        ], cde.name, cde.datatype, cde.required, cde.members, cde.validation
