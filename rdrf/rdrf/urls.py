...
from rdrf.auth.forms import (
    RDRFLoginAssistanceForm,
    RDRFPasswordResetForm,
    RDRFSetPasswordForm,
)
...
if settings.DEBUG is True:
    normalpatterns += [
        ...
        re_path(
            r"^testAppError", handler_application_error, name="test application error"
        ),
...
integration_patterns = [
    ...
    re_path(
        r"^externaldemographics/(?P<registry_code>\w+)/(?P<patient_id>\d+)/?$",
...
proms_patterns = [
    re_path(
        r"^promslanding/?$", PromsLandingPageView.as_view(), name="proms_landing_page"
    ),
    re_path(r"^proms/?$", PromsView.as_view(), name="proms"),
    re_path(
        r"^promscompleted/?$", PromsCompletedPageView.as_view(), name="proms_completed"
    ),
    re_path(
        r"^api/proms/v1/",
        include(
            ("rdrf.services.rest.urls.proms_api_urls", "proms_api_urls"), namespace=None
        ),
    ),
]
...
proms_site_patterns = [
    re_path(
        r"^promsqrcode/(?P<patient_token>[0-9A-Za-z_\-]+)/?$",
        PromsQRCodeImageView.as_view(),
        name="promsqrcode",
    )
]
...
normalpatterns += [
    ...
    re_path(
        r"^translations/jsi18n/$",
        JavaScriptCatalog.as_view(),
        name="javascript-catalog",
    ),
    re_path(
        r"^iprestrict/", include(("iprestrict.urls", "iprestrict_urls"), namespace=None)
    ),
    re_path(
        r"^useraudit/",
        include(
    ...
    re_path(
        r"^customactions/(?P<action_id>\d+)/(?P<patient_id>\d+)/?$",
        CustomActionView.as_view(),
        name="custom_action",
    ),
    re_path(
        r"^api/v1/",
        include(("rdrf.services.rest.urls.api_urls", "api_urls"), namespace="v1"),
    ),
    ...
    re_path(
        r"^password_change/?$", ChangePasswordView.as_view(), name="password_change"
    ),
    re_path(
    ...
    re_path(
        r"^login_assistance_confirm/(?P<uidb64>[0-9A-Za-z_\-]+)/(?P<token>[0-9A-Za-z]{1,13}-[0-9A-Za-z]{1,20})/?$",
        login_assistance_confirm,
    ...
    re_path(
        r"^(?P<registry_code>\w+)/(?P<patient_id>\d+)/clinicalproms/?$",
        PromsClinicalView.as_view(),
    ...
    re_path(
        r"^import/?",
        import_registry_view.ImportRegistryView.as_view(),
        name="import_registry",
    ),
    re_path(
        r"^reportdatatable/(?P<query_model_id>\d+)/?$",
        report_view.ReportDataTableView.as_view(),
    ...
    re_path(
        r"^contexts/(?P<registry_code>\w+)/(?P<patient_id>\d+)/add/(?P<context_form_group_id>\d+)?$",
        RDRFContextCreateView.as_view(),
        name="context_add",
    ),
    re_path(
        r"contexts/(?P<registry_code>\w+)/(?P<patient_id>\d+)/(?P<context_id>\d+)/edit/?$",
        RDRFContextEditView.as_view(),
        name="context_edit",
    ),
    re_path(r"^router/", login_router.RouterView.as_view(), name="login_router"),
    re_path(
        r"^(?P<registry_code>\w+)/forms/(?P<form_id>\w+)/(?P<patient_id>\d+)/(?P<context_id>add)/?$",
    ...
    re_path(
        r"^(?P<registry_code>\w+)/forms/switchlock/(?P<form_id>\w+)/(?P<patient_id>\d+)/(?P<context_id>\d+)?$",
        form_view.FormSwitchLockingView.as_view(),
        name="registry_form_switchlock",
    ),
    re_path(
        r"^(?P<registry_code>\w+)/forms/print/(?P<form_id>\w+)/(?P<patient_id>\d+)/(?P<context_id>\d+)?$",
        form_view.FormPrintView.as_view(),
        name="registry_form_print",
    ),
    re_path(
    ...
    re_path(
        r"^(?P<registry_code>\w+)/?$",
        registry_view.RegistryView.as_view(),
        name="registry",
    ),
    re_path(
        r"^(?P<registry_code>\w+)/patient/add/?$",
        patient_view.AddPatientView.as_view(),
        name="patient_add",
    ),
    re_path(
        r"^(?P<registry_code>\w+)/patient/query/?$",
        patient_view.QueryPatientView.as_view(),
        name="patient_query",
    ),
    re_path(
        r"^(?P<registry_code>\w+)/patient/(?P<patient_id>\d+)/edit$",
        patient_view.PatientEditView.as_view(),
        name="patient_edit",
    ),
    re_path(
        r"^(?P<registry_code>\w+)/permissions/?$",
        PermissionMatrixView.as_view(),
        name="permission_matrix",
    ),
    # ---- Consent related URLs -----------------
    re_path(
        r"^(?P<registry_code>\w+)/consent/?$",
        consent_view.ConsentList.as_view(),
        name="consent_list",
    ),
    re_path(
        r"^(?P<registry_code>\w+)/consent/(?P<section_id>\d+)/(?P<patient_id>\d+)/?$",
        consent_view.ConsentDetails.as_view(),
        name="consent_details",
    ),
    re_path(
        r"^(?P<registry_code>\w+)/consent/print/?$",
        consent_view.PrintConsentList.as_view(),
        name="print_consent_list",
    ),
    re_path(
        r"^(?P<registry_code>\w+)/(?P<patient_id>\d+)/consents/?$",
        form_view.CustomConsentFormView.as_view(),
        name="consent_form_view",
    ),
    re_path(
        r"^(?P<registry_code>\w+)/(?P<patient_id>\d+)/consents/print/?$",
        consent_view.ConsentDetailsPrint.as_view(),
        name="print_consent_details",
    ),
    # ---- Clinician related URLs -----------------
    re_path(
        r"^(?P<registry_code>\w+)/(?P<patient_id>\d+)/clinician/?$",
        clinician_view.ClinicianFormView.as_view(),
        name="clinician_form_view",
    ),
    re_path(
        r"^(?P<registry_code>\w+)/verifications/?$",
        PatientsRequiringVerificationView.as_view(),
        name="verifications_list",
    ),
    re_path(
        r"^(?P<registry_code>\w+)/verifications/(?P<patient_id>\d+)/(?P<context_id>\d+)/?$",
        PatientVerificationView.as_view(),
        name="patient_verification",
    ),
    re_path(
        r"^clinicianactivate/(?P<activation_key>\w+)/?$",
        clinician_view.ClinicianActivationView.as_view(),
        name="clinician_activate",
    ),
    # ---- Email Notifications URLs -------------
    re_path(
        r"^resend_email/(?P<notification_history_id>\w+)/?$",
        ResendEmail.as_view(),
        name="resend_email",
    ),
    # -------------------------------------------
    re_path(
        r"^(?P<registry_code>\w+)/familylinkage/(?P<initial_index>\d+)?$",
        FamilyLinkageView.as_view(),
        name="family_linkage",
    ),
    re_path(
        r"^(?P<registry_code>\w+)/questionnaire/(?P<questionnaire_context>\w+)?$",
        form_view.QuestionnaireView.as_view(),
        name="questionnaire",
    ),
    re_path(
        r"^(?P<registry_code>\w+)/approval/(?P<questionnaire_response_id>\d+)/?$",
        form_view.QuestionnaireHandlingView.as_view(),
        name="questionnaire_response",
    ),
    re_path(
        r"^(?P<registry_code>\w+)/uploads/(?P<file_id>([0-9a-fA-F]{24})|(\d+))$",
        form_view.FileUploadView.as_view(),
        name="file_upload",
    ),
    re_path(
        r"^admin/lookups/",
        include(("ajax_select.urls", "ajax_select_urls"), namespace=None),
    ),
    re_path(
        r"^questionnaireconfig/(?P<form_pk>\d+)/?$",
        form_view.QuestionnaireConfigurationView.as_view(),
        name="questionnaire_config",
    ),
    re_path(
        r"api/familylookup/(?P<reg_code>\w+)/?$",
        FamilyLookup.as_view(),
        name="family_lookup",
    ),
    re_path(
        r"api/patientlookup/(?P<reg_code>\w+)/?$",
        PatientLookup.as_view(),
        name="patient_lookup",
    ),
    # ---- Look-ups URLs -----------------------
    re_path(
        r"^lookup/username/(?P<username>[\w.%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,4})/?$",
        UsernameLookup.as_view(),
        name="lookup_username",
    ),
    re_path(
        r"^lookup/recaptcha/?$",
        RecaptchaValidator.as_view(),
        name="recaptcha_validator",
    ),
    # -------------------------------------------
    re_path(
        r"^activate/complete/?$",
...
if settings.REGISTRATION_ENABLED:
    registry_urls = [
        ...
        re_path(
            r"^register/failed/?$",
            TemplateView.as_view(template_name="registration/registration_failed.html"),
